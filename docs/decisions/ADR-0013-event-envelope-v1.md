...
- tenant-scoped **비-run** 이벤트(`tenant.policy.updated.v1` 등)에서 `run_id`의 nullable 허용 여부 — **종결(rev.2, 아래): tenant 분기는 `run_id` required 유지. 비-run tenant 토픽이 실제로 등장하면 해당 토픽을 도입하는 ADR에서 재개정한다.**
...
## rev.2 개정 (2026-07-12, 사용자 승인 — Wave 1 계획 승인 문답)

본 절은 **개정 기록(amendment)**이다 — 위 본문(Purpose~부록)과 상단 `Status` 헤더 값은 수정하지 않는다. W1 authoritative 스키마(`packages/contracts/json-schema/envelope/v1/event-envelope.schema.json`)가 아래 4건을 구현한다. Wave 1 계획 승인 문답(`/Users/edmond104/.claude/plans/virtual-cooking-map.md` §1.2 항목 2)에서 4건 일괄 승인되었다.

### ① `payload` — envelope의 10번째 구조 멤버

원 본문(§Current decision)이 확정한 9 공통 필드에 더해, `payload`를 **10번째 구조 멤버**로 명문화한다: 공통 9필드 + `context_type` discriminator + `payload` 컨테이너(object) — 이 10개가 envelope 최상위 구조를 구성한다. `payload`는 **전 `context_type` 분기(tenant/system/aggregate)에서 required**이며, 빈 객체(`{}`)는 허용한다(발행 시점에 도메인 payload가 아직 없는 경우를 위한 최소 유효값 — 부재가 아니라 "빈 컨테이너"로 표현). 이는 §부록의 예시 3종이 이미 보여준 `payload` 키 사용을 구조 차원에서 확정하는 것으로, 새로운 필드 도입이 아니라 기존 예시의 암묵 구조를 명문화한 것이다.

### ② `occurred_at` — Z-종결 표기만 허용 (표기 정준화)

`occurred_at`은 RFC3339 UTC 중 **`Z`-종결 표기만 허용**한다 — `+00:00` 오프셋 표기(RFC3339상 UTC와 동치)는 **거부**한다. 이는 envelope가 `frozen` class(ADR-0012)이므로 정준(canonical) 표현을 하나로 통일해 구조 diff·canonical-JSON 비교가 표기 변형을 의미 없는 diff로 오탐하지 않도록 하기 위함이다. §부록 예시 3종은 이미 전부 `Z`-종결(`...T09:14:32Z` 등)이므로 본 정준화는 기존 예시와 정합하며 예시 변경을 요구하지 않는다.

### ③ `schema_version` — prerelease/build 메타데이터 배제 확정

`schema_version`(semver string, §Current decision 표 4행)은 **prerelease/build 메타데이터를 배제**한다 — 즉 `MAJOR.MINOR.PATCH` 순수형만 허용하고 `1.0.0-rc.1`, `1.0.0+build.5` 형태는 거부한다. ADR-0012의 호환성 정책(major/minor 판정)이 순수 3-부분 semver를 전제하므로, prerelease/build 태그가 섞이면 태그 비교·N-1 tag 해석(ADR-0011 `contracts/{name}/vX.Y.Z`)이 모호해진다 — 이를 스키마 레벨에서 원천 차단한다.

### ④ tenant 분기 `run_id` required 유지 — Open decisions 항목 1 종결

§Current decision 표(tenant 행)의 "run-scoped 이벤트는 필수(non-run tenant 이벤트의 nullable 허용 여부는 OPEN)" 서술 중 OPEN 부분을 다음과 같이 종결한다: **tenant 분기는 `run_id`를 required로 유지**한다(nullable 허용하지 않음). 비-run tenant 토픽(예: `tenant.policy.updated.v1`류)이 실제로 카탈로그에 등장하면, 그 개별 토픽을 도입하는 ADR에서 필요 시 재개정한다 — 본 ADR에서 선제적으로 예외를 열어두지 않는다. 위 "## Open decisions" 항목 1을 이 결론으로 갱신했다(해당 1줄만 수정, 본 개정 절 추가 외 나머지 본문은 append-only).
