"""context/site-context/v1 fixture validation (w1-11)."""
...
SCHEMA_PATH = (
    CONTRACTS_JSON_SCHEMA_DIR / "context" / "site-context" / "v1" / "site-context.schema.json"
)
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "site-context"
...
def test_domain_url_violation_rejected_by_pattern() -> None:
    """Spot check: the VERIFIED FINDING in the schema's own $comment says
    format:hostname is annotation-only in this repo's tooling and the
    `pattern` is the actually-enforcing check -- prove the URL-shaped
    domain fixture is rejected (by pattern, not silently accepted).
    """
    validator = _validator()
    fixture_path = FIXTURE_DIR / "invalid" / "domain-is-url-not-hostname.json"
    data = strip_metadata(load_json(fixture_path))
    errors = list(validator.iter_errors(data))
    assert errors, "expected the URL-shaped domain fixture to fail validation"
