"""domain/source-snapshot/v1 fixture validation (w1-11)."""
...
SCHEMA_PATH = (
    CONTRACTS_JSON_SCHEMA_DIR / "domain" / "source-snapshot" / "v1" / "source-snapshot.schema.json"
)
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "source-snapshot"
...
def test_inline_content_rejected_as_additional_property() -> None:
    validator = _validator()
    fixture_path = FIXTURE_DIR / "invalid" / "inline-content.json"
    data = strip_metadata(load_json(fixture_path))
    errors = list(validator.iter_errors(data))
    messages = " ".join(e.message for e in errors)
    assert "inline_content" in messages or "additional" in messages.lower(), (
        f"expected an additionalProperties/inline_content-referencing error, got: {messages}"
    )
