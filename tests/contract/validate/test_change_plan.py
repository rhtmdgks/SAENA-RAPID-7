"""domain/change-plan/v1 fixture validation (w1-11)."""
...
SCHEMA_PATH = (
    CONTRACTS_JSON_SCHEMA_DIR / "domain" / "change-plan" / "v1" / "change-plan.schema.json"
)
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "change-plan"
...
def test_fixture_inventory_complete() -> None:
    ...
    assert len(INVALID_FIXTURES) == 7, f"found {[p.name for p in INVALID_FIXTURES]}"
...
def test_approval_false_violation_is_const() -> None:
    """Spot check: approval_required's const:true invariant (Plan Gate)
    must be the actual reason approval-false.json is rejected.
    """
    validator = _validator()
    fixture_path = FIXTURE_DIR / "invalid" / "approval-false.json"
    data = strip_metadata(load_json(fixture_path))
    errors = list(validator.iter_errors(data))
    messages = " ".join(e.message for e in errors)
    assert "True" in messages or "true" in messages.lower(), (
        f"expected a const:true-referencing error, got: {messages}"
    )
