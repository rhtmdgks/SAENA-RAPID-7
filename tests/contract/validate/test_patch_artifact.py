"""domain/patch-artifact/v1 fixture validation (w1-11)."""
...
SCHEMA_PATH = (
    CONTRACTS_JSON_SCHEMA_DIR / "domain" / "patch-artifact" / "v1" / "patch-artifact.schema.json"
)
FIXTURE_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "patch-artifact"
...
def test_inline_diff_rejected_as_additional_property() -> None:
    validator = _validator()
    fixture_path = FIXTURE_DIR / "invalid" / "inline-diff.json"
    data = strip_metadata(load_json(fixture_path))
    errors = list(validator.iter_errors(data))
    messages = " ".join(e.message for e in errors)
    assert "inline_diff" in messages or "additional" in messages.lower(), (
        f"expected an additionalProperties/inline_diff-referencing error, got: {messages}"
    )
